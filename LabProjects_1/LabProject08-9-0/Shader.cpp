...
CPlayerShader::~CPlayerShader()
{
#ifndef _WITH_SCENE_ROOT_SIGNATURE
...
void CPlayerShader::CreateShader(ID3D12Device *pd3dDevice, ID3D12RootSignature *pd3dGraphicsRootSignature, UINT nRenderTargets)
{
	...
	CShader::CreateShader(pd3dDevice, m_pd3dGraphicsRootSignature, nRenderTargets);
}

ID3D12RootSignature* CPlayerShader::CreateGraphicsRootSignature(ID3D12Device *pd3dDevice)
{
...
void CObjectsShader::BuildObjects(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, void *pContext)
{
	...
	CreateShaderResourceViews(pd3dDevice, pTexture, 0, ROOT_PARAMETER_TEXTURE2DARRAY);
...
void CPostProcessingShader::CreateShader(ID3D12Device *pd3dDevice, ID3D12RootSignature *pd3dGraphicsRootSignature, UINT nRenderTargets)
{
	...
	m_nRenderTargets = nRenderTargets;
	m_pd3dRtvCPUDescriptorHandles = new D3D12_CPU_DESCRIPTOR_HANDLE[m_nRenderTargets];

	m_pdxgiRtvFormats = new DXGI_FORMAT[m_nRenderTargets];
	for (int i = 0; i < m_nRenderTargets; i++) m_pdxgiRtvFormats[i] = DXGI_FORMAT_R8G8B8A8_UNORM;

	m_pTexture = new CTexture(m_nRenderTargets, RESOURCE_TEXTURE2D, 0, 1);

	m_nPipelineStates = 1;
	m_ppd3dPipelineStates = new ID3D12PipelineState*[m_nPipelineStates];

	CShader::CreateShader(pd3dDevice, m_pd3dGraphicsRootSignature, m_nRenderTargets);
}
...
void CPostProcessingShader::BuildObjects(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, void *pContext)
{
	...
	for (int i = 0; i < m_nRenderTargets; i++)
	{
		d3dClearValue.Format = m_pdxgiRtvFormats[i];
		m_pTexture->CreateTexture(pd3dDevice, FRAME_BUFFER_WIDTH, FRAME_BUFFER_HEIGHT, m_pdxgiRtvFormats[i], D3D12_RESOURCE_FLAG_ALLOW_RENDER_TARGET, D3D12_RESOURCE_STATE_COMMON, &d3dClearValue, RESOURCE_TEXTURE2D, i);
	}
	...
	D3D12_CPU_DESCRIPTOR_HANDLE d3dRtvCPUDescriptorHandle = *((D3D12_CPU_DESCRIPTOR_HANDLE*)pContext);
	for (int i = 0; i < m_nRenderTargets; i++)
	{
		d3dRenderTargetViewDesc.Format = m_pdxgiRtvFormats[i];
		m_pd3dRtvCPUDescriptorHandles[i] = d3dRtvCPUDescriptorHandle;
		ID3D12Resource* pd3dTextureResource = m_pTexture->GetResource(i);
		pd3dDevice->CreateRenderTargetView(pd3dTextureResource, &d3dRenderTargetViewDesc, m_pd3dRtvCPUDescriptorHandles[i]);
...
void CPostProcessingShader::OnPreRenderTarget(ID3D12GraphicsCommandList* pd3dCommandList, int nRenderTargets, D3D12_CPU_DESCRIPTOR_HANDLE *pd3dRtvCPUHandles, D3D12_CPU_DESCRIPTOR_HANDLE d3dDepthStencilBufferDSVCPUHandle)
{
	D3D12_CPU_DESCRIPTOR_HANDLE *pd3dAllRtvCPUHandles = new D3D12_CPU_DESCRIPTOR_HANDLE[nRenderTargets + m_nRenderTargets];
	...
	pd3dCommandList->OMSetRenderTargets(nRenderTargets + m_nRenderTargets, pd3dAllRtvCPUHandles, FALSE, &d3dDepthStencilBufferDSVCPUHandle);
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//
CPostProcessingByMonochromeShader::CPostProcessingByMonochromeShader()
{
}
...
D3D12_SHADER_BYTECODE CPostProcessingByMonochromeShader::CreatePixelShader(ID3DBlob **ppd3dShaderBlob)
{
	return(CShader::CompileShaderFromFile(L"Shaders.hlsl", "PSPostProcessingByMonochrome", "ps_5_1", ppd3dShaderBlob));
}

void CPostProcessingByMonochromeShader::CreateShader(ID3D12Device* pd3dDevice, ID3D12RootSignature* pd3dGraphicsRootSignature, UINT nRenderTargets)
{
