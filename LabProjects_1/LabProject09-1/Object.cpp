...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// 
C4x4GridPatchObject::C4x4GridPatchObject(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, ID3D12RootSignature* pd3dGraphicsRootSignature) : CGameObject(1)
{
	C4x4GridPatchMesh* p4x4GridPatchMesh = new C4x4GridPatchMesh(pd3dDevice, pd3dCommandList);
	SetMesh(0, p4x4GridPatchMesh);

	CreateShaderVariables(pd3dDevice, pd3dCommandList);

	CGridQuadPatchShader* pGridQuadPatchShader = new CGridQuadPatchShader();
	pGridQuadPatchShader->CreateShader(pd3dDevice, pd3dGraphicsRootSignature);
	pGridQuadPatchShader->CreateShaderVariables(pd3dDevice, pd3dCommandList);
	pGridQuadPatchShader->CreateCbvSrvDescriptorHeaps(pd3dDevice, 1, 0);
	pGridQuadPatchShader->CreateConstantBufferViews(pd3dDevice, 1, m_pd3dcbGameObject, ((sizeof(CB_GAMEOBJECT_INFO) + 255) & ~255));

	SetCbvGPUDescriptorHandle(pGridQuadPatchShader->GetGPUCbvDescriptorStartHandle());

	SetShader(pGridQuadPatchShader);
}

C4x4GridPatchObject::~C4x4GridPatchObject()
{
}