...
void CObjectsShader::BuildObjects(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, void *pContext)
{
	...
	CreateShaderResourceViews(pd3dDevice, pTexture, 0, ROOT_PARAMETER_TEXTURE);
...
void CPostProcessingShader::BuildObjects(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, void *pContext)
{
	...
	m_pTexture->CreateTexture(pd3dDevice, FRAME_BUFFER_WIDTH, FRAME_BUFFER_HEIGHT, DXGI_FORMAT_R8G8B8A8_UNORM, D3D12_RESOURCE_FLAG_NONE, D3D12_RESOURCE_STATE_COMMON, NULL, RESOURCE_TEXTURE2D, 0);
...
void CPostProcessingShader::UpdateTextureResource(ID3D12GraphicsCommandList* pd3dCommandList, ID3D12Resource* pd3dRenderTarget)
{
	ID3D12Resource* pd3dTextureResource = m_pTexture->GetResource(0);
	::SynchronizeResourceTransition(pd3dCommandList, pd3dRenderTarget, D3D12_RESOURCE_STATE_RENDER_TARGET, D3D12_RESOURCE_STATE_COPY_SOURCE);
	::SynchronizeResourceTransition(pd3dCommandList, pd3dTextureResource, D3D12_RESOURCE_STATE_COMMON, D3D12_RESOURCE_STATE_COPY_DEST);

	pd3dCommandList->CopyResource(pd3dTextureResource, pd3dRenderTarget);

	::SynchronizeResourceTransition(pd3dCommandList, pd3dTextureResource, D3D12_RESOURCE_STATE_COPY_DEST, D3D12_RESOURCE_STATE_COMMON);
	::SynchronizeResourceTransition(pd3dCommandList, pd3dRenderTarget, D3D12_RESOURCE_STATE_COPY_SOURCE, D3D12_RESOURCE_STATE_RENDER_TARGET);
}
