/*
	...
	Depth:				정규화된 Z값맞춰주기위해 사용 (0.0 ~ 1.0)
	...
	Root Signature:		리소스들이 그래픽스 파이프라인의 셰이더들에서 어떻게 사용될 것인지 정의하는 배열 (셰이더 시그니처라고도 함)
						시그니처 하나당 64개의 32비트 원소(루트 파라미터)로 구성
	Root Parameter:		루트 상수(int p), 루트 서술자(int* p), 서술자 테이블(int** p) -> 셰이더의 전역 변수
...
/*
	...
	-> 복사가 끝나면(커맨드 큐가 비어있으면) 업로드 힙은 제거, 텍스쳐 리소스는 업로드 힙에 만들 수 없음
	...
	정점 버퍼 뷰, 인덱스 버퍼 뷰는 내부적으로 서술자, 서술자 힙이 필요없음 -> Create*View()가 없음
	GPU의 코어는 CPU 대비 아주 많음
	루트 시그니처는 여러개 생성할 수 있지만, Set은 하나만 가능
*/
...
/*
	...
	정점 셰이더:		월드 변환 -> 카메라 변환 -> 투영 변환 -> 스키닝 -> 모핑 -> 조명 처리(정점 조명, 픽셀 조명) (정점마다 호출)
	래스터라이저:		원근 투영 나누기 -> 컬링 -> 클리핑 -> 프리미티브를 픽셀로 변환 (이 때, 선형보간 수행)-> 뷰포트 맵핑(스크린 변환을 대체)
	픽셀 셰이더:		픽셀 색상 결정 (픽셀마다 호출)
