...
D3D12_SHADER_BYTECODE CCubeShader::CreateVertexShader(ID3DBlob** ppd3dShaderBlob)
{
	return CShader::CompileShaderFromFile(L"Shaders.hlsl", "VSInstancing", "vs_5_1", ppd3dShaderBlob);
}

D3D12_SHADER_BYTECODE CCubeShader::CreatePixelShader(ID3DBlob** ppd3dShaderBlob)
{
	return CShader::CompileShaderFromFile(L"Shaders.hlsl", "PSInstancing", "ps_5_1", ppd3dShaderBlob);
}

void CCubeShader::CreateShader(ID3D12Device* pd3dDevice, ID3D12RootSignature* pd3dGraphicsRootSignature)
{
	m_nPipelineStates = 1;
	m_ppd3dPipelineStates = new ID3D12PipelineState * [m_nPipelineStates];
...
void CCubeShader::ReleaseShaderVariables()
{
...
void CCubeShader::BuildObjects(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList)
{
	m_nObjects = 10;
	...
	{
		CCubeObject* pCubeObject{};
		pCubeObject = new CCubeObject();
		pCubeObject->SetPosition(float(rand() % 100 - 50), float(rand() % 100 - 50), float(rand() % 100));
		pCubeObject->SetMovingDirection(XMFLOAT3(float((rand() % 10 + 1) / 10.0f), float((rand() % 10 + 1) / 10.0f), 0.0f));
		pCubeObject->SetMovingSpeed(float(rand() % 6 + 5));
		pCubeObject->SetRotationAxis(XMFLOAT3(1.0f, 1.0f, 1.0f));
		pCubeObject->SetRotationSpeed(10.0f * float(rand() % 5 + 1));
...
void CCubeShader::Render(ID3D12GraphicsCommandList* pd3dCommandList, CCamera* pCamera)
{
